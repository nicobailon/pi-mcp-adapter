...
/**
 * Render MCP tool call display with server name and tool name.
 */
function renderMcpToolCall(
    baseLabel: string,
    accentText: string,
    theme: Theme,
    resourceUri?: string,
    options?: { showColon?: boolean }
): Text {
    const separator = options?.showColon !== false ? ": " : " ";
    const text = theme.fg("toolTitle", theme.bold(`${baseLabel}${separator}`))
        + theme.fg("accent", accentText)
        + (resourceUri ? theme.fg("dim", " (resource)") : "");
...
function renderMcpToolResult(
    ...
    export default function mcpAdapter(pi: ExtensionAPI) {
        ...
        pi.registerTool({
            ...
            renderCall(args, theme: Theme) {
                let suffix: string;
                if (args.tool) suffix = args.tool;
                else if (args.connect) suffix = `connect ${args.connect}`;
                else if (args.describe) suffix = `describe ${args.describe}`;
                else if (args.search) suffix = `search "${args.search}"`;
                else if (args.server) suffix = `server ${args.server}`;
                else suffix = "status";
