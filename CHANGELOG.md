...
- Collapsed `mcpScript` rows in compact rendering are titled `mcpScript` and list the MCP tools the script called, from its call trace with repeat counts, instead of showing only the first line of output. Failed traced operations show as a `✗N` status after the title, which narrow rows keep by shrinking the title; script-supplied paths that are not terminal-safe are shown as escaped strings. Script code is still never copied into the row.
