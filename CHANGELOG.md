...
- Collapsed `mcpScript` rows in compact rendering are titled `mcpScript` (`mcpScript ✗N` when N traced operations failed, so narrow rows keep the count) and list the MCP tools the script called, from its call trace with repeat counts and terminal-sanitized paths, instead of showing only the first line of output. Script code is still never copied into the row.
