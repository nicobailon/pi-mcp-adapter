...
### Fixed
- macOS Keychain and Linux Secret Service no longer split ordinary OAuth records into Windows-sized chunks. Chunking stays in place for Windows Credential Manager and the size-limited test store. Existing chunked records are compacted on the next non-status read. This stops Atlassian-sized tokens from spawning a Keychain prompt per chunk, which also made Always Allow useless because each write minted new item names.
