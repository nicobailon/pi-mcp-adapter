...
- Namespace proxy tool names now hex-encode only unsafe characters, so server names containing `.` (for example `awslabs.aws-documentation-mcp-server`) no longer exceed the 64-character tool-name limit enforced by Bedrock. Thanks to [@saikpr](https://github.com/saikpr) for PR #529.
