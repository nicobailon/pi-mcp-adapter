...
- MCP proxy tool can now expose a per-server tool inventory to the model via prompt guidelines (`settings.toolInventory: "names"` or `"descriptions"`), the middle tier between the opaque proxy (discover tools on demand) and direct tools (full schemas loaded up front). Names and one-line descriptions are read from the metadata cache without connecting, and the inventory stays out of the gateway description so the cached prompt prefix remains stable.
