...
- `bearerToken: "!command"` and other bearer-token shellouts are now re-resolved on every HTTP request via a TTL-cached per-request `FetchLike`, so a rotating JWT (e.g. Cloudflare Access 24h session) stays valid for the lifetime of a lazy-keep-alive connection instead of locking the transport into a permanent 401 until pi is restarted. Static, env-var, and stored bearer tokens now also trigger a fresh reconnect when a keep-alive refresh sees 401, instead of staying in failure backoff with the stale token.
