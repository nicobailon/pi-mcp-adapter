...
Successful intermediate data has a **16 MiB cumulative UTF-8 JSON transfer budget per script** across sequential and parallel calls. A result exceeding the remaining budget returns `{ ok: false, error: { code: "intermediate_result_too_large", message } }`; rejected calls do not consume the budget, but upstream side effects may already have happened. This is not a memory limit: responses, serialization, copies, concurrency, and script values still allocate memory, and synchronous serialization can delay deadline handling.
