...
When `settings.jev.scriptEvaluation` is explicitly enabled, `await jev.evaluate({ state, questions, sources })` performs one bounded host-side evaluation. Declare every MCP server represented in state; only `settings.jev.allowedServers` may leave the machine. Use `tools.search({ query, searchMode: "semantic" })` only when semantic search is enabled. Never treat confidence or probability as approval: later `tools.call` operations retain their normal approval gate. For action loops, require fresh observations and positive step/time/evaluation budgets, validate the operation and target, and stop on uncertainty, stale candidates, no progress, no match, or missing information. Never automatically retry an action that may have produced a side effect.
